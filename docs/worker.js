...
function analyze(draws){
 ...
 draws.forEach(r=>{if(r.d>=limit)r.n.forEach(n=>freq[n]++);});
 const arr=[...Array(60)].map((_,i)=>({n:i+1,c:freq[i+1]})).sort((a,b)=>b.c-a.c);
 cached={q:arr.slice(0,20),m:arr.slice(20,40),f:arr.slice(40)};
 self.postMessage({type:'stats',result:{h:cached.q.slice(0,6).map(x=>x.n),m:cached.m.slice(0,6).map(x=>x.n),f:cached.f.slice(0,6).map(x=>x.n)}});
}
